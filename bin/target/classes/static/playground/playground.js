...
var playground = myApp.controller('PlaygroundCtrl', ['$rootScope','CoursesService','$scope','$location','$cookieStore', '$routeParams', '$http', function ($rootScope,CoursesService,$scope,$location,$cookieStore, $routeParams, $http) {
        ...
        $rootScope.cmt = function(courArg){
			$cookieStore.put('courArg', courArg);
			$rootScope.courArg = courArg;
		}
        
        
        $rootScope.userInfo = function(coursCmt){
            $("#message" + coursCmt.cId).hide();
        	
        	CoursesService.user(coursCmt.userId)
            .then(
						function(success) {
							var user = success.data;
		                    $rootScope.coursCmt = angular.extend(coursCmt, user);		
						},
						function(error) {
        ...
        $rootScope.userReplyInfo = function(replyCmt){
        	
        	CoursesService.user(replyCmt.userId)
            .then(
						function(success) {
							var user = success.data;
		                    $rootScope.replyCmt = angular.extend(replyCmt, user);		
						},
						function(error) {
        ...
        $rootScope.cmtText = function(cId,coursCmt){
        	
        	 if($cookieStore.get('messageId')){
        		 var messageId = $cookieStore.get('messageId');
        		 $(messageId).hide();
        		 $cookieStore.remove('messageId');
        		 
        	 }

                $("#message" + cId).show();
                $cookieStore.put('messageId', "#message" + cId);
    			$cookieStore.put('coursCmt', coursCmt);
    			$rootScope.coursCmt = coursCmt;

        };

        
        $scope.saveCmt = function(cText,coursArgsId,userId){
        	CoursesService.saveCmt(cText,coursArgsId,userId)
        	.then(
					function(success) {
						//$rootScope.coursesLoad();
						$rootScope.courArg.coursCmts.push(success.data);
						$cookieStore.remove('courArg');
						$cookieStore.put('courArg', $rootScope.courArg);
        ...
        $scope.saveReplyCmt = function(cText,cId,coursArgsId,userPostId,userReplyId){
        	CoursesService.saveReplyCmt(cText,cId,coursArgsId,userPostId,userReplyId)
        	.then(
					function(success) {
						//$rootScope.coursesLoad();
						$rootScope.coursCmt.replyCmts = $rootScope.coursCmt.replyCmts || [];
						$rootScope.coursCmt.replyCmts.push(success.data);
						$cookieStore.remove('coursCmt');
						$cookieStore.put('coursCmt', $rootScope.coursCmt);
						$rootScope.coursCmt = $cookieStore.get('coursCmt');						
						$scope.creplyText='';
